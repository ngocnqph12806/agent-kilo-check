...
export function StepDone() {
  ...
  return (
    <div className="flex flex-col items-center px-4 py-12 text-center">
      <div className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full bg-brand-hero-soft text-primary shadow-brand-card">
        <PartyPopper className="h-9 w-9" aria-hidden />
      </div>
      <h1 className="text-4xl font-extrabold tracking-tight text-brand-strong sm:text-5xl">
      ...
      <ul className="mt-10 grid w-full max-w-md gap-3 text-left">
        {['Profile created', 'Skills seeded for matching', '50 starter seeds claimed', 'Email verified'].map(
          (item) => (
            <li
              key={item}
              className="flex items-center gap-3 rounded-xl border border-brand-default bg-card p-3 text-sm shadow-brand-card"
            >
              <CheckCircle2 className="h-5 w-5 shrink-0 text-primary" aria-hidden />
              <span className="font-medium text-brand-strong">{item}</span>
            </li>
          )
        )}
      </ul>
